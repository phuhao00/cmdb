...
| POST | `/api/v1/assets/:id/tags` | 添加资产标签 |
| DELETE | `/api/v1/assets/:id/tags/:tag` | 移除资产标签 |
| GET | `/api/v1/assets/tags` | 获取所有标签 |
| POST | `/api/v1/assets/search` | 高级资产搜索 |
| GET | `/api/v1/assets/departments` | 获取所有部门 |
| GET | `/api/v1/assets/owners` | 获取所有负责人 |
...
| 方法 | 端点 | 描述 |
|------|------|------|
| GET | `/api/v1/audit-logs` | 搜索审计日志 |
| GET | `/api/v1/audit-logs/user/:userId` | 获取用户活动日志 |
| GET | `/api/v1/audit-logs/resource/:type/:id` | 获取资源变更历史 |
| GET | `/api/v1/audit-logs/stats` | 获取审计日志统计 |
...
### v1.2.1 (2025年1月)
- ✅ 修复Docker端口映射配置（8081:8081）
- ✅ 修复前端API配置，支持Docker环境
- ✅ 添加缺失的资产导出API端点（/assets/export）
- ✅ 添加缺失的资产历史记录API端点（/assets/:id/history）
- ✅ 完善资产历史记录功能，包括模型、存储库和应用层
- ✅ 修复CORS配置，支持更多前端地址
- ✅ 确保所有API端点与前端调用匹配
